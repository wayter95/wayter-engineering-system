...
const rules = [
  ...
  { re: /\bgit\s+commit\b[^\n]*(Co-Authored-By|Co-authored-by|Generated with|Claude|Anthropic|noreply@anthropic\.com|🤖)/i, why: "commit com atribuição de IA; .agents/git.md exige commits sem trailer e sem assinatura do agente" },
  { re: /\bgit\s+commit\b[^|;&]*\s--(author|trailer)\b/, why: "git commit --author/--trailer altera autoria ou adiciona trailer" },
  { re: /\bgit\s+add\s+(-A|--all|\.|\*)(\s|$)/, why: "git add -A/. adiciona tudo sem revisão; adicione arquivos explicitamente (.agents/git.md)" },
  { re: /\bgit\s+(commit|add)\b[^|;&]*\s-a(m)?\b/, why: "git commit -a adiciona tudo sem revisão; adicione arquivos explicitamente" },
